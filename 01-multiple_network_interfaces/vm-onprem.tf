resource "google_service_account" "onprem" {
  account_id   = "onprem"
  display_name = "Custom SA for the onprem VM Instance"
}

resource "google_project_iam_member" "onprem" {
  for_each = toset([
    "roles/logging.logWriter",
    "roles/monitoring.metricWriter",
  ])
  project = var.project_id
  member  = google_service_account.onprem.member
  role    = each.value
}
...
resource "google_compute_instance" "onprem" {
  ...
  network_interface {
    ...
    subnetwork = google_compute_subnetwork.subnets["onprem-main"].id
  }
