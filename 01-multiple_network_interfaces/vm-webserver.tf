resource "google_service_account" "webserver" {
  ...
  display_name = "Custom SA for the webserver VM Instance"
}

resource "google_project_iam_member" "webserver" {
  for_each = toset([
    "roles/logging.logWriter",
    "roles/monitoring.metricWriter",
  ])
  project = var.project_id
  member  = google_service_account.webserver.member
  role    = each.value
}
...
resource "google_compute_instance" "webserver" {
  ...
  network_interface {
    ...
    subnetwork = google_compute_subnetwork.subnets["web-main"].id
  }
